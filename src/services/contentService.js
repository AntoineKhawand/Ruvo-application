...
// Bump this when tip content changes. Triggers a background re-seed so
// Firestore stays in sync without manual Firebase Console intervention.
const TIPS_VERSION = 2;
...
export const FALLBACK_TIPS = [
    ...
    },
    {
        ...
        steps: [
            { title: 'Tall Spine', desc: 'Ears over shoulders, shoulders over hips. Maintain this while running.' },
            { title: 'Forward Lean', desc: 'Lean 5° forward from the ankles (not the waist). Use gravity to propel you.' },
    ...
    },
    {
        ...
        steps: [
            ...
            { title: 'Add Electrolytes', desc: 'For runs over 60 min, add sodium to prevent hyponatremia.' },
        ],
        viewCount: 14200,
    },
    {
    ...
    },
    {
        ...
        steps: [
            { title: 'Easy Run', desc: 'Run at 60–65% max heart rate. You should talk in full sentences.' },
    ...
    // ── MENTAL ───────────────────────────────────────────────────
    {
        ...
        steps: [
            ...
            { title: 'Anchor Word', desc: 'Pick one word (e.g. "power"). Repeat it rhythmically with your stride.' },
        ],
        viewCount: 10500,
    },
    {
    ...
    // ── STRENGTH ─────────────────────────────────────────────────
    {
        ...
        steps: [
            { title: 'Plank Hold', desc: '3 x 45-second planks. Squeeze everything: glutes, quads, and shoulders.' },
    ...
    },
    {
        ...
        steps: [
            { title: 'Bulgarian Split Squat', desc: '3 x 8 each leg. The single best exercise for runners — builds everything.' },
            { title: 'Romanian Deadlift', desc: '3 x 10 to strengthen hamstrings and glutes that power your stride.' },
            { title: 'Calf Raises', desc: '3 x 20 single-leg. Calves absorb 8x body weight with every footfall.' },
        ],
        viewCount: 9700,
    },
    {
        ...
        steps: [
            ...
            { title: 'Sync Up', desc: 'Fall into the group rhythm to conserve energy.' },
    ...
    },
    {
        ...
        steps: [
            ...
            { title: 'Reflective Vest', desc: 'Wear over your top. Choose 360° reflective strips.' },
    ...
    // ── RACE PREP ────────────────────────────────────────────────
    {
        ...
        steps: [
            { title: 'Calculate Your Target', desc: 'Your race pace should feel almost embarrassingly easy for the first 2 km.' },
    ...
    // ── INJURY PREVENTION ────────────────────────────────────────
    {
        ...
        steps: [
            ...
            { title: 'Increase by Max 10%', desc: "If you're running 20 km/week, max increase is 2 km next week." },
...
export const contentService = {
    ...
    /**
     * Returns the full tip list with Firestore view counts merged in.
     *
     * FALLBACK_TIPS is always the source of truth for tip content — this
     * ensures new tips added in code are immediately visible to all users
     * without requiring a manual Firestore migration.
     *
     * Firestore is used only for persisting real-time view counts.
     * If Firestore is stale (fewer docs than FALLBACK_TIPS), a background
     * upsert runs to bring it up to date.
     */
    fetchTips: async () => {
        try {
            const snap = await getDocs(collection(db, CONTENT_COLLECTION));

            // Build a view-count map from whatever Firestore has
            const storedCounts = {};
            snap.docs.forEach(d => {
                storedCounts[d.id] = d.data().viewCount ?? 0;
            });

            // If Firestore is missing tips (or has stale v1 data), reseed in background
            if (snap.size < FALLBACK_TIPS.length || snap.docs.some(d => !d.data().category)) {
                contentService.seedTipsToFirestore().catch(() => {});
            }

            // Return FALLBACK_TIPS enriched with real view counts
            return FALLBACK_TIPS.map(tip => ({
                ...tip,
                viewCount: storedCounts[tip.id.toString()] ?? tip.viewCount,
            }));
        } catch {
            // Offline or rules error — FALLBACK_TIPS always works
    ...
    incrementTipView: async (tipId) => {
        try {
            await setDoc(
                doc(db, CONTENT_COLLECTION, tipId.toString()),
                { viewCount: increment(1) },
                { merge: true }
            );
        } catch {
            // Non-critical — view counts are cosmetic
        }
    },

    /**
     * Upsert all tips to Firestore using merge:true so existing view counts
     * are preserved while content fields are refreshed.
     * Versioned so it only runs when TIPS_VERSION changes.
     */
    seedTipsToFirestore: async () => {
        try {
            ...
            FALLBACK_TIPS.forEach(tip => {
                ...
                // merge: true preserves viewCount while updating all content fields
                batch.set(ref, {
                    ...tip,
                    type: 'tip',
            ...
            // Write version marker so we can detect future stale data
            batch.set(doc(db, CONTENT_COLLECTION, '_meta'), {
                tipsVersion: TIPS_VERSION,
                totalTips: FALLBACK_TIPS.length,
                seededAt: new Date().toISOString(),
            }, { merge: true });
            await batch.commit();
            return true;
        } catch (e) {
            console.warn('[contentService] seed failed:', e.message);
            return false;
        }
    },
};
