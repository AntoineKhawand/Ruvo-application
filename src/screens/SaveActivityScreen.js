...
export default function SaveActivityScreen({ route, navigation }) {
  ...
  const { updateUserProfile, userData } = useUser(); 
  ...
  const handleSave = async () => {
    ...
    // 2. Calculate New User Stats
    const currentTotalKm = userData.totalKm || 0;
    const currentCoins = userData.coins || 0;
    const currentXP = userData.currentXP || 0;

    const earnedCoins = Math.floor(newActivity.distance * 10); // 10 coins per km
    const earnedXP = Math.floor(newActivity.distance * 50); // 50 XP per km

    // 3. Prepare Update Data
    const updates = {
        runHistory: [newActivity, ...(userData.runHistory || [])], // Add to top of list
        totalKm: currentTotalKm + newActivity.distance,
        coins: currentCoins + earnedCoins,
        currentXP: currentXP + earnedXP,
        weeklyDistance: (userData.weeklyDistance || 0) + newActivity.distance
    };

    // 4. Save to Firebase via Context
    try {
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.aiInsightCard}>
            ...
            <Text style={styles.aiInsightBody}>
                {(runData.effortRating || runData.rpe) >= 8 
                    ? "High intensity effort detected. You've significantly boosted your anaerobic threshold. Ensure 48h recovery before your next hard session."
                    : "Excellent aerobic base building. You maintained a consistent heart rate zone, which is perfect for long-term endurance efficiency."}
            </Text>
        </View>
