...
export default function ClubDetailScreen({ route, navigation }) {
    const { userData, toggleClubMembership, followUser, unfollowUser, clubFeeds, addClubPost, addClubComment, sendFriendRequest, cancelFriendRequest, addTemporaryUsers, updateClub, deleteClub, toggleClubPostLike } = useUser();
    ...
    useEffect(() => {
        const initialFeed = clubFeeds[clubData.id] || [];
    ...
    const handlePost = (type = 'text') => {
        ...
        const newPost = { 
            ...
            event: isReminder ? { time: `${reminderData.date}, ${reminderData.time}`, loc: reminderData.location } : null 
        };
        setFeedItems([newPost, ...feedItems]); 
        addClubPost(clubData.id, newPost); 
        setNewPostText(''); 
    ...
    // --- FIX: TOGGLE LIKE WITH PERSISTENCE ---
    const toggleLike = (id) => { 
        // 1. Update Local UI instantly
        setFeedItems(prev => prev.map(item => { 
            if (item.id === id) { 
                const newLiked = !item.liked; 
                return { ...item, liked: newLiked, likes: newLiked ? item.likes + 1 : item.likes - 1 }; 
            } 
            return item; 
        })); 
        
        // 2. Persist to Global Context
        toggleClubPostLike(clubData.id, id);
    };
