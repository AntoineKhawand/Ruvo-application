...
export default function ChatScreen({ route, navigation }) {
    ...
    const { userData, setUserData, sendMessage, blockUser, detectLocation } = useUser();
    const [inputText, setInputText] = useState('');
    const [showMenu, setShowMenu] = useState(false);
    const flatListRef = useRef();

    // 1. Get User Details
    // Fallback if user isn't found (e.g., deleted account)
    const user = (userData.allUsers || []).find(u => u.id === userId) || { name: 'Unknown User', avatar: null };
    
    // 2. Get Chat History (Safe Access)
    const messages = (userData.chats && userData.chats[userId]) ? userData.chats[userId] : [];
    ...
    const handleMenuOption = (action) => {
        setShowMenu(false);
        if (action === 'Clear') {
            // Update Local State directly for clearing
            setUserData(prev => {
