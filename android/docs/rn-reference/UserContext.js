...
// --- 8. PUSH NOTIFICATIONS ---
  ...
  const addRunToHistory = async (runEntry, calculatedUpdates = {}) => {
    ...
    try {
      ...
      // waiting for the Firestore onSnapshot round-trip. Skip if the onSnapshot listener
      // already delivered this run (same id) while the Cloud Function call was in flight.
      setUserData(prev => {
        const alreadyPresent = (prev.runHistory || []).some(r => r.id === runEntry.id);
